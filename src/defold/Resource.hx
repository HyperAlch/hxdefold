...
import defold.Render.RenderTargetFlags;
import defold.types.TextureResourceHandle;
import haxe.extern.EitherType;
import defold.Go.GoPlayback;
import defold.types.RenderResourceHandle;
...
extern final class Resource {
	...
	**/
	...
	/**
		...
		@param path A hash, string, or resource handle pointing to the render target
		@return Information about the render target
	**/
	@:native('get_render_target_info')
	static function getRenderTargetInfo(path:HashOrStringOrRenderResourceHandle):RenderTargetInformation;
}
...
/**
	All the information returned by the `Resource.getRenderTargetInfo` method
**/
typedef RenderTargetInformation = {
	var handle:RenderResourceHandle;
	var width:Int;
	var height:Int;
	var depth:Int;
	var mipmaps:Int;
...
/**
	Resource Buffer type used in the return value of `Resource.getRenderTargetInfo` field.
**/
@:native("_G.resource")
extern enum abstract ResourceBufferType(Int) to Int {
	@:native('BUFFER_TYPE_COLOR0')
	var TypeColor0;

	@:native('BUFFER_TYPE_COLOR1')
	var TypeColor1;

	@:native('BUFFER_TYPE_COLOR2')
	var TypeColor2;

	@:native('BUFFER_TYPE_COLOR3')
	var TypeColor3;

	@:native('BUFFER_TYPE_DEPTH')
	var TypeDepth;

	@:native('BUFFER_TYPE_STENCIL')
	var TypeStencil;
}
...
/**
	Shortcut type that can be either a Hash, String, or RenderResourceHandle 
**/
typedef HashOrStringOrRenderResourceHandle = EitherType<Hash, EitherType<String, RenderResourceHandle>>;
